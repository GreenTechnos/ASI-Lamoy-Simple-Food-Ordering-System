...
export const resetPasswordProfile = async (newPassword, confirmPassword) => {
  const token = localStorage.getItem("token"); // or sessionStorage based on your login flow

  const response = await fetch(`${API_BASE_URL}/user/profile/reset-password`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
